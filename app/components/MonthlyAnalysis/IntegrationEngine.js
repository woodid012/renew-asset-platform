...
export const generateCompleteMonthlyCashFlowAnalysis = async (
  ...
  try {
    ...
    // Phase 2: Generate time periods
    const assetStartYear = new Date(asset.assetStartDate).getFullYear();
    const constructionStartYear = assetStartYear - Math.ceil(financialParams.constructionDuration / 12);
    ...
    for (let year = constructionStartYear; year <= analysisEndYear; year++) {
      for (let month = 1; month <= 12; month++) {
        ...
        const assetStart = new Date(assetStartYear, 0, 1);
        const constructionStart = new Date(constructionStartYear, 0, 1);
        ...
        if (phase !== 'pre-construction') {
          monthlyPeriods.push({
            ...
            monthIndex: monthlyPeriods.length
          });
        }
      }
    }
    
    console.log(`Generated ${monthlyPeriods.length} monthly periods from ${constructionStartYear} to ${analysisEndYear}`);
    ...
    projectFinanceResults.debtServiceSchedule.forEach((debtMonth, index) => {
      ...
      const yearsSinceStart = debtMonth.year - assetStartYear;
