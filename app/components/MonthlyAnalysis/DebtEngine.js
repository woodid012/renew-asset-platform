...
export const calculateMonthlyOpex = (asset, financialParams, year, month, assetStartYear) => {
...
export const shouldApplyGracePeriod = (phase, assetStartYear, year, month, constructionMonths) => {
  ...
  if (phase === 'operations') {
    const assetStartDate = new Date(assetStartYear, 0, 1); // Assuming Jan 1 start for simplicity
...
export const solveOptimalDebt = (
  ...
  while (iterations < maxIterations && (upperBound - lowerBound) > tolerance) {
    ...
    const testResult = testDebtFeasibility(
      currentDebt,
      monthlyRevenueData,
      financialParams,
      assetStartYear,
...
export const calculateConstructionCashFlows = (
  ...
  if (financialParams.equityTimingUpfront) {
    ...
    constructionCashFlows.push({
      monthIndex: 0,
      year: assetStartYear - Math.ceil(constructionMonths / 12),
      month: 1,
      phase: 'construction',
      monthlyCapexSpend,
      monthlyDebtDrawdown,
    ...
    for (let i = 0; i < constructionMonths; i++) {
      const year = assetStartYear - Math.ceil((constructionMonths - i) / 12);
