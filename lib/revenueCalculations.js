...
export const applyEscalation = (basePrice, year, constants) => {
  // Hardcoded escalation settings for now
  const escalation = 2.5; // 2.5% annual escalation
  const referenceYear = 2025; // Reference year
  
  if (!basePrice) return basePrice;
  const yearDiff = year - referenceYear;
  return basePrice * Math.pow(1 + escalation / 100, yearDiff);
};
...
export const calculateRenewablesRevenue = (asset, timeInterval, year, quarter, assetStartYear, constants, getMerchantPrice) => {
  ...
  const merchantGreenPrice = applyEscalation(
    getMerchantPrice(profile, 'green', asset.state, timeInterval) || 35, 
    year, 
    constants
  );
  const merchantEnergyPrice = applyEscalation(
    getMerchantPrice(profile, 'Energy', asset.state, timeInterval) || 65, 
    year, 
    constants
  );
...
export const calculateStorageRevenue = (asset, timeInterval, year, assetStartYear, getMerchantPrice, constants) => {
  ...
  if (merchantPercentage > 0) {
    ...
    // Get and escalate prices using storage profile
