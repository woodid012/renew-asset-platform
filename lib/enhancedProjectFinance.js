...
// UPDATED: Enhanced project finance calculations with proper partial period handling
...
/**
 * Calculate project finance for individual asset with enhanced cash flow structure
 */
const calculateAssetProjectFinance = async (asset, assetCostData, portfolioTimeSeries, constants) => {
  ...
  const debtResults = calculateEnhancedDebtMetrics(
    ...
    assetCostData,
    constants,
    assetStartMonth // Pass start month for partial period calculations
  )
  ...
  return {
    ...
    debtAnalysis: {
      ...
      debtServiceByQuarter: debtResults.debtServiceByQuarter, // NEW: Quarterly breakdown
      fullyRepaid: debtResults.fullyRepaid,
      gracePeriods: debtResults.gracePeriods // NEW: Track grace periods
    },
    ...
    cashFlowAnalysis: assetCashFlows.map((cf, index) => ({
      ...
      // NEW: Quarterly breakdown if available
...
/**
 * ENHANCED: Calculate debt metrics with proper partial period handling
 */
const calculateEnhancedDebtMetrics = (cashFlows, costData, constants, assetStartMonth) => {
  ...
  // Solve for optimal gearing using binary search with partial period logic
  ...
  return {
    ...
    debtServiceByQuarter: optimalDebtResult.debtServiceByQuarter, // NEW
    fullyRepaid: optimalDebtResult.fullyRepaid,
    gracePeriods: optimalDebtResult.gracePeriods, // NEW
    iterations: optimalDebtResult.iterations
  }
}

/**
 * ENHANCED: Solve for optimal debt with partial period grace logic
 */
  ...
  while (iterations < maxIterations && (upperBound - lowerBound) > tolerance) {
    ...
    if (schedule.fullyRepaid && schedule.minDSCR >= Math.min(...targetDSCRs) * 0.95) {
      ...
      bestResult = {
        ...
        debtServiceByQuarter: schedule.debtServiceByQuarter, // NEW
        fullyRepaid: schedule.fullyRepaid,
        gracePeriods: schedule.gracePeriods, // NEW
        iterations
      }
    } else {
...
/**
 * ENHANCED: Calculate debt schedule with partial period grace logic
 */
const calculateDebtScheduleWithPartialPeriods = (debtAmount, cashFlows, interestRate, targetDSCRs, assetStartMonth) => {
  ...
  for (let yearIndex = 0; yearIndex < tenorYears; yearIndex++) {
    ...
    for (let quarter = 1; quarter <= 4; quarter++) {
      const quarterCashFlow = operatingCashFlow / 4 // Assume even distribution
      ...
      if (debtBalance[yearIndex] > 0 && !isPartialQuarter) {
        ...
        // Update balance for next quarter (simplified - should be more granular)
        debtBalance[yearIndex] = Math.max(0, debtBalance[yearIndex] - quarterlyPrincipal)
      } else if (isPartialQuarter) {
        // Grace period - track but no payment
        gracePeriods.push({
          year: currentYear,
          quarter: quarter,
...
/**
 * ENHANCED: Determine if grace period should apply based on asset start month
 */
const shouldApplyGracePeriod = (assetStartMonth, quarter) => {
  // Define which months belong to which quarters
  const quarterMonths = {
  ...
  // Find which quarter the asset starts in
  ...
  // - If asset starts in month 1 of quarter: no grace (full quarter)
  // - If asset starts in month 2 or 3 of quarter: grace period for that quarter
  ...
  if (quarter === startQuarterNum && monthIndexInQuarter > 0) {
    // Partial quarter - give grace period
    return true
  }
...
/**
 * Initialize enhanced project finance values
 */
const initializeEnhancedProjectValues = (assets) => {
  ...
  return portfolioTimeSeries
    ...
    .map(period => {
      ...
      return {
        ...
        opex: -0.15 * assetData.revenue.totalRevenue,
...
export { initializeEnhancedProjectValues }