...
const calculateEnhancedDebtMetrics = (cashFlows, costData, constants, assetStartMonth) => {
  ...
  // Filter for operational cash flows to calculate target DSCRs
  const operationalCashFlows = cashFlows.filter(cf => cf.phase === 'operations');

  // Calculate target DSCRs for each operational year based on revenue mix
  const targetDSCRs = operationalCashFlows.slice(0, tenorYears).map(cf => {
  ...
  // Solve for optimal gearing using binary search, passing all cash flows
  const optimalDebtResult = solveOptimalDebtWithPartialPeriods(
    cashFlows, // Pass all cash flows, including construction
    capex,
    maxGearing,
    interestRate,
    targetDSCRs,
...
const solveOptimalDebtWithPartialPeriods = (cashFlows, capex, maxGearing, interestRate, targetDSCRs, assetStartMonth) => {
  ...
  // Define minOverallTargetDSCR and dscrTolerance
  const minOverallTargetDSCR = Math.min(...targetDSCRs);
  const dscrTolerance = 0.05; // Allow DSCR to be slightly below target for max debt
  ...
  while (iterations < maxIterations && (upperBound - lowerBound) > tolerance) {
    ...
    if (schedule.fullyRepaid && schedule.minDSCR >= minOverallTargetDSCR - dscrTolerance) {
      // This debt amount is feasible, try to go higher
      ...
      // This debt amount is not feasible (not repaid or DSCR too low), try lower
...
const calculateDebtScheduleWithPartialPeriods = (debtAmount, cashFlows, interestRate, targetDSCRs, assetStartMonth) => {
  ...
  const dscrValues = []; // Store DSCR only for operational periods
  const gracePeriods = [];
  ...
  for (let yearIndex = 0; yearIndex < tenorYears; yearIndex++) {
    ...
    const phase = cashFlows[yearIndex].phase; // Get phase from cashFlows

    let yearlyDebtService = 0;
    let runningDebtBalance = debtBalance[yearIndex];
    ...
    for (let quarter = 1; quarter <= 4; quarter++) {
      ...
      if (phase === 'construction') {
        // During construction, only accrue interest
        quarterlyInterest = runningDebtBalance * quarterlyRate;
        runningDebtBalance += quarterlyInterest; // Interest accrues to balance
        gracePeriods.push({
          year: currentYear,
          quarter: quarter,
      ...
      } else if (runningDebtBalance > 0 && !isPartialQuarter) {
        // Operational phase, normal debt service
        quarterlyInterest = runningDebtBalance * quarterlyRate;
        const targetDSCR = targetDSCRs[yearIndex] || 1.5; // Use target DSCR for operational periods
        ...
        runningDebtBalance = Math.max(0, runningDebtBalance - quarterlyPrincipal);
      } else if (isPartialQuarter) {
        // Grace period in operational phase
        gracePeriods.push({
          year: currentYear,
          quarter: quarter,
        ...
        runningDebtBalance += runningDebtBalance * quarterlyRate; // Interest still accrues
      }
      ...
      debtServiceByQuarter.push({
        ...
        quarterlyInterest,
        quarterlyPrincipal,
        isGracePeriod: (phase === 'construction' || isPartialQuarter),
        quarterCashFlow,
        remainingBalance: runningDebtBalance
      });
    ...
    // Only calculate DSCR for operational periods
    if (phase === 'operations' && yearlyDebtService > 0) {
      dscrValues.push(operatingCashFlow / yearlyDebtService);
    } else if (phase === 'operations' && yearlyDebtService <= 0 && operatingCashFlow > 0) {
      dscrValues.push(Infinity); // Positive cash flow, no debt service
    } else if (phase === 'operations') {
      dscrValues.push(0); // No operating cash flow or negative debt service
    }

    if (yearIndex < tenorYears) {
      debtBalance[yearIndex + 1] = runningDebtBalance;
    }
  }
  ...
  const minDSCR = dscrValues.length > 0 ? Math.min(...dscrValues.filter(d => d !== null && isFinite(d) && d > 0)) : null;
  ...
  return {
    ...
    fullyRepaid,
    averageDebtService,
    minDSCR: isFinite(minDSCR) ? minDSCR : null,
    gracePeriods
  };
};
  ...
  return portfolioTimeSeries
    .map(period => {
      ...
      return {
        ...
        phase: assetData.phaseInfo?.phase // Include phase information
      }
    })
    .filter(cf => cf !== null)
}

const calculateEnhancedEquityCashFlows = (costData, debtResults, allCashFlows) => {
  const equityCashFlows = [];
  const totalEquityRequired = costData.capex - debtResults.debtAmount; // Total equity needed for the project

  // Track actual equity invested during construction periods
  let actualConstructionEquity = 0;

  allCashFlows.forEach((cf, index) => {
    let equityCF = 0;
    const debtServiceThisPeriod = debtResults.debtServiceByYear[index] || 0;

    if (cf.phase === 'construction') {
      // Equity contribution during construction is monthly investment minus monthly debt drawdown
      // This should be a negative value (cash outflow)
      equityCF = -(cf.monthlyInvestment - cf.monthlyDebtDrawdown);
      actualConstructionEquity += equityCF; // Accumulate negative values
    } else if (cf.phase === 'operations') {
      // During operations, equity cash flow is operating cash flow minus debt service
      equityCF = cf.operatingCashFlow - debtServiceThisPeriod;
    }

    // Add terminal value in the final period if applicable
    if (index === allCashFlows.length - 1 && costData.terminalValue) {
      equityCF += costData.terminalValue;
    }
    equityCashFlows.push(equityCF);
  });

  // Now, adjust the equity cash flows to ensure the total equity matches totalEquityRequired
  // This adjustment is crucial for IRR calculation accuracy.
  // The difference between totalEquityRequired and actualConstructionEquity (sum of negative values)
  // needs to be accounted for, typically in the first period.

  const initialEquityAdjustment = totalEquityRequired + actualConstructionEquity;

  // Find the first period (usually the first construction period or first operational period if no construction)
  // and apply the remaining equity investment there.
  if (equityCashFlows.length > 0) {
    equityCashFlows[0] -= initialEquityAdjustment; // Subtract, as initialEquityAdjustment is positive for required equity
  }
